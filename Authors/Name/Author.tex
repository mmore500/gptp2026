...
% Provide a clean TOC author entry so svmult's bookmark hook does not
% try to expand \and through hyperref's PDF-string converter (which
% errors out under older hyperref/bookmark releases).  \tocauthor is
% a \newtoks register in svmult.cls, so assign with toks syntax.
\tocauthor={Matthew Andres Moreno, Luis Zaman, Emily Dolson}

% Also disable the orcidlink/email/at helpers when hyperref does try
% to expand \institute or \author into a PDF string.
\makeatletter
