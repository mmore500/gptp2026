...
%
Ongoing advances in high-performance computing (HPC) continue to burgeon the quantities of cores available across both traditional CPU-based nodes, but also increasingly in compute-dense hardware accelerator devices.
...
In the context of digital evolution, these technologies offer immense opportunity for both hypothesis-driven experiments exploring multi-scale biological phenomena and application-oriented evolutionary computation tackling difficult fitness landscapes or expensive fitness evaluations.
A key challenge in harnessing available compute, however, is bottlenecks in memory and bandwidth --- a trend epitomized by emerging next-generation computational hardware accelerators targeting AI/ML workloads.
